    ...
    case "UPDATEAPP":
      return {
        ...
        apps: state.apps.map((app) =>
          app.ref.value.id === action.payload.ref.value.id
            ? action.payload
            : app
        ),
        error: null,
...
const AppsProvider = ({ children }) => {
  ...
  const addApp = async (userclient, app) => {
    try {
      const res = await userclient.query(q.Call("addjobapplication", app));
  ...
  const updateApp = async (userclient, appref, app) => {
    try {
      const res = await userclient.query(
        q.Update(q.Ref(q.Collection("applications"), appref.value.id), {
          data: app,
        })
      );
